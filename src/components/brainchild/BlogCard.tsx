    ...
    >
      ...
      <div className="p-5 flex flex-col gap-2 font-body">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-secondary bg-secondary/10 px-2 py-0.5 rounded-full">{post.author}</span>
          <span className="text-xs text-muted-foreground">{new Date(post.publishedAt).toLocaleDateString()}</span>
        </div>
        <h3 className="text-lg md:text-2xl font-heading font-bold text-foreground group-hover:text-primary transition-colors duration-300 leading-tight">{post.title}</h3>
        <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed">{post.excerpt}</p>
        <a href={`/blog/${post.slug}`} className="text-primary font-semibold text-sm hover:opacity-80 transition inline-flex items-center gap-1 group-hover:gap-2 duration-300 mt-1">
