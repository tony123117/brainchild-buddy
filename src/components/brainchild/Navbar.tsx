...
export function Navbar() {
  ...
  return (
    <nav className="flex justify-between items-center px-4 md:px-8 pt-5 pb-2.5 sticky top-0 z-50 bg-background font-heading">
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-background shadow-lg lg:hidden z-50 p-6 flex flex-col gap-4">
          {["Home", "About Us", "Admissions", "Blog", "Careers", "Contact us"].map((item) => (
            <a key={item} href="/" className="text-lg hover:text-primary transition-colors" onClick={() => setMenuOpen(false)}>
              {item}
            </a>
          ))}
